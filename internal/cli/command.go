...
func newMirrorCommand(state *cobraState) *cobra.Command {
	...
	experience := group("experience", "Close visible Mirror experience episodes", "A Mirror experience records what was sensed, how the stack reacted, what outcome was observed, and what lesson remains. Completed episodes become future Mirror context. Helpful or corrected outcomes project response targets. Harmful and inconclusive outcomes project the visible lesson as an outcome-conditioned reflection target without supervising the failed attempt. Every outcome can also project portable memory to the separate Hermes runtime lane.")
	experience.AddCommand(
		leaf(state, "observe <outcome.json>", "Record an outcome and reflect on a prior Mirror reaction", "The strict outcome names an open episode and declares HELPFUL, CORRECTED, HARMFUL, or INCONCLUSIVE feedback. The command appends outcome and reflection events to --ledger. --learn-to writes either a helpful/corrected response target or, for harmful/inconclusive experience, a structured outcome-conditioned reflection whose failed attempt remains unsupervised. --hermes-to writes a portable episodic-memory capsule; it does not claim the separate Hermes runtime was executed.", cobra.ExactArgs(1), runMirrorExperienceObserve,
		...
		leaf(state, "project-trajectory <draft.json>", "Seal observed simulator experience and project its lesson", "The strict draft records one observed attempt, result, and lesson. WALDO seals and appends the source record to --record-to, then appends an outcome-conditioned reflection to --learn-to. The attempt is evidence only; only the lesson is supervised. This command does not train a model or change weights.", cobra.ExactArgs(1), runMirrorExperienceProjectTrajectory,
			requiredTextFlag("record-to", "append sealed observed trajectory records to this private JSONL file"),
			requiredTextFlag("learn-to", "append training-ready reflection projections to this private JSONL file")),
	)
	command.AddCommand(
		...
		leaf(state, "workspace <root> <request.json>", "Run the local neural model through a bounded project-filesystem Hand", "The selected root is the sensory boundary. WALMI can list, read, stat, and hash only inside it. Writes require --allow-write and use the transactional candidate writer with hashes, limits, rollback, and symlink/root-escape refusal. Verification commands come only from the strict request file and execute without a shell. The loop has no install, integration, promotion, learning, or CANON authority. --experience-to visibly appends the complete receipt as a private candidate record; it does not train or mutate weights.", cobra.ExactArgs(2), runMirrorWorkspace,
			requiredTextFlag("model", "local WALDO model used for the workspace loop"),
			booleanFlag("allow-write", "allow the bounded transactional writer inside the selected root"),
			integerFlag("max-tokens", 1024, "maximum tokens per neural tool-call response"),
			decimalFlag("temperature", 0.1, "tool-call sampling temperature"),
			decimalFlag("top-p", 0.95, "tool-call nucleus sampling probability"),
			unsigned64Flag("seed", 0, "deterministic tool-call sampling seed"),
			integerFlag("timeout-seconds", 600, "whole-loop timeout in seconds (1..3600)"),
			textFlag("experience-to", "", "append the complete private workspace receipt as an experience candidate")),
		ground,
		experience,
	)
	return command
