...
func (resolver PyTorchResolver) Resolve(ctx context.Context, request ResolveRequest) (Selection, error) {
	...
	return Selection{}, fmt.Errorf("no usable PyTorch runtime found; choose the CUDA, ROCm, or CPU install for this host at https://pytorch.org/get-started/locally/%s", detail)
}
...
func probePyTorch(ctx context.Context, python string) (pyTorchProbe, error) {
	...
	if runtime.GOOS == "windows" {
		// A first import from a portable runtime can spend significant time
		// creating its drive-local bytecode cache before the probe executes.
