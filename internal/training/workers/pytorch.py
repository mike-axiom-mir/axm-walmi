...
def commit_directory(temporary, destination):
    for root, _, files in os.walk(temporary):
        for name in files:
            # Windows requires a writable handle for fsync even when the file
            # contents were already written and this pass is only a durability
            # barrier.
            descriptor = os.open(os.path.join(root, name), os.O_RDWR)
            try:
                os.fsync(descriptor)
            finally:
                os.close(descriptor)
    # Windows cannot fsync directory handles opened through os.open. The
    # checkpoint files themselves have already been flushed above, so retain
    # the stronger directory metadata barrier on platforms that support it.
    if os.name != "nt":
