...
func syncContributionTree(root string) error {
	return filepath.WalkDir(root, func(path string, entry os.DirEntry, walkErr error) error {
		...
		// Windows requires a writable handle for FlushFileBuffers. These are
		// newly staged contribution files, so opening read/write preserves the
		// same durability guarantee on every supported host.
