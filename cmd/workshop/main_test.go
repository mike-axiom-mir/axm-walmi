...
func TestFailedChatPersistsDeliveryErrorAndRetriesWithoutDuplicate(t *testing.T) {
	...
	t.Setenv("AXM_WORKSHOP_DATA", t.TempDir())
	...
	if len(a.state.Sessions) != 1 {
		t.Fatalf("seeded sessions=%d, want 1", len(a.state.Sessions))
	}
	sessionID := a.state.Sessions[0].ID
	...
	failed := post(map[string]any{"SessionID": sessionID, "Text": "Try the local model"})
	...
	retried := post(map[string]any{"SessionID": sessionID, "RetryMessageID": messageID})
